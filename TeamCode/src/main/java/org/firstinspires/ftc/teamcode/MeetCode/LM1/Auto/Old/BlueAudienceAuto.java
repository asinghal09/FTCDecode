...
public class BlueAudienceAuto extends LinearOpMode{
    ...
    private static final Logger log = LoggerFactory.getLogger(BlueAudienceAuto.class);
    ...
    public void runOpMode(){
        ...
        Pose2d initialPose = mirror(new Pose2d(55, 17, Math.toRadians(-4)));
        ...
        Action toGoalwPreload = drive.actionBuilder(initialPose)
                .setReversed(true)
                .splineToSplineHeading(mirror(new Pose2d(-30, 27 ,Math.toRadians(-52))), Math.toRadians(128))
                .build();

        Action to1stLine = drive.actionBuilder(mirror(new Pose2d (-26, 23, Math.toRadians(-52))))
                .setReversed(false)
                .splineToSplineHeading(mirror(new Pose2d(-16,28, Math.toRadians(85))),Math.PI/2, new TranslationalVelConstraint(20) )
                .build();
        ...
        Action toGoalWFirstLine = drive.actionBuilder(mirror(new Pose2d(-16, 45, Math.toRadians(85))))
                ...
                .splineToLinearHeading(mirror(new Pose2d(-30,27,Math.toRadians(-52))), 26*Math.PI/35)
                .build();

        Action to2ndLine = drive.actionBuilder(mirror(new Pose2d (-30, 27, Math.toRadians(-52))))
                .setReversed(false)
                .splineToSplineHeading(mirror(new Pose2d(7,28, Math.toRadians(85))),Math.PI/2, new TranslationalVelConstraint(20) )
                .build();
